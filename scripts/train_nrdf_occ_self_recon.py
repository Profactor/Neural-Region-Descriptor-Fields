...
if __name__ == '__main__':
    ...
    p.add_argument('--stage1_folder', default=None, help='Checkpoint to trained model.')
    ...
    elif opt.stage1_folder is not None:
        model_ckpt_path = os.path.join(opt.logging_root, opt.stage1_folder, 'checkpoints')
        selected_model = sorted(glob.glob(model_ckpt_path+'/model*best.pth'))[-1]
        model.load_state_dict(torch.load(selected_model))
        # update checkpoint path for saving config 
    ...
    training.train_imdc_point_onetdata_inv_desc(model=model_parallel, inv_model=inv_model, train_dataloader=train_loader,
