...
from nrdf.model.inverse_descriptor_fn import InvDescAllGloblaLocalFn
...
if __name__ == '__main__':
    ...
    p.add_argument('--obj_class', type=str, required=True, help='bottle, mug, bowl, all')
    ...
    p.add_argument('--inv_checkpoint_path', default=None, help='Checkpoint to trained model.')
    p.add_argument('--desc_loss', action='store_true', help='whether to use descriptor loss function')
    p.add_argument('--drop_last', action='store_true', help='whether to use dcd loss function instead of basic cd')
    p.add_argument('--desc_cosine_loss', action='store_true', help='whether to use descriptor cosine similarity loss function')
    p.add_argument('--sm_radius', type=float, default=0.1, help='radius for smooth loss')
    p.add_argument('--stage2_folder', default=None, help='Checkpoint to trained model.')
    ...
    train_loader = DataLoader(train_data, num_workers=0, batch_size=opt.batch_size, shuffle=True, drop_last=opt.drop_last)
    ...
    val_loader = DataLoader(val_data, num_workers=0, batch_size=opt.batch_size, shuffle=True, drop_last=opt.drop_last)
    print('val len -', len(val_loader))                                

    # -- CREATE MODEL -- #
    sigmoid = True if opt.data_type == 'occ' else False
    model = IMDCPointOnet(return_features=True, encode_position=opt.position_encode, sigmoid=sigmoid).cuda()
    inv_model = InvDescAllGloblaLocalFn(z_global_dim=128, desc_dim=1953).cuda()
    print(model, inv_model)

    # -- LOAD CHECKPOINT --#
    if opt.stage2_folder is not None:
        model_ckpt_path = os.path.join(opt.logging_root, opt.stage2_folder, 'checkpoints')    
    if opt.checkpoint_path is not None:
        model.load_state_dict(torch.load(opt.checkpoint_path))
    elif opt.stage2_folder is not None:
        selected_model = sorted(glob.glob(model_ckpt_path+'/model*best_sr_shape.pth'))[-1]
        model.load_state_dict(torch.load(selected_model))    
        opt.selected_checkpoint_path = selected_model        
    if opt.inv_checkpoint_path is not None:
        inv_model.load_state_dict(torch.load(opt.inv_checkpoint_path))
    elif opt.stage2_folder is not None:
        selected_inv_model = sorted(glob.glob(model_ckpt_path+'/inv_model*best_sr_shape.pth'))[-1]
        inv_model.load_state_dict(torch.load(selected_inv_model))      
        opt.selected_inv_checkpoint_path = selected_inv_model
    ...
    sr_loss = val_sr_loss = losses.selfrec_loss
    cr_loss = val_cr_loss = losses.selfrec_emd_chamf_normal_smooth_loss
    if opt.desc_loss:
        d_loss = val_d_loss = losses.desc_loss_fn
    else:
        d_loss = val_d_loss = None

    training.train_imdc_point_onetdata_inv_desc_cr(model=model_parallel, inv_model=inv_model, train_dataloader=train_loader,
        ...
        sr_loss_fn=sr_loss, val_sr_loss_fn=val_sr_loss,
        cr_loss_fn=cr_loss, val_cr_loss_fn=val_cr_loss, d_loss_fn=d_loss, val_d_loss_fn=val_d_loss,
        config_dict=config, sm_radius=opt.sm_radius)
