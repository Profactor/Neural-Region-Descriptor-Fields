# REF: https://github.com/liufeng2915/Implicit_Dense_Correspondence/blob/main/loss.py
import torch
...
radius = 0.0625
nsample =  16
def smooth_loss(esti_shapes, shapes, radius= radius):
...
def smooth_loss_m(esti_shapes, shapes, radius= radius):
    _,_,_,idx = sample_and_group(npatch, radius, nsample, torch.cat((shapes[1:,:,:], shapes[:1,:,:])))
    offset_vectors = esti_shapes - shapes # assuming same index for the esti shapes and shapes
    patch_offset_vectors = index_points(offset_vectors.unsqueeze(-1), idx).squeeze(-1)
    loss_smooth = torch.mean(1-F.cosine_similarity(patch_offset_vectors[:,:,:-1,:], patch_offset_vectors[:,:,1:,:], 3))
    return loss_smooth
